    ...
    <div className='p-6 flex flex-col gap-4 bg-background border border-border rounded-lg'>
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        <div className='flex-1'>
          ...
          <Label htmlFor="skills" className="text-sm text-muted-foreground mt-2">
            Add relevant skills for the job position.
          </Label>
        </div>
        <div className='flex-1 flex flex-col gap-2'>
          <div className='flex gap-2'>
            <Input type='text' id="skills" value={newSkill} onChange={(e)=>setNewSkill(e.target.value)} className='flex-1' placeholder='Enter a skill'/>
          ...
          <div className='flex flex-wrap gap-2 mt-2'>
            {skills.map((skill:string,index:number)=>(
              <div key={index} className='bg-secondary text-secondary-foreground px-3 py-1 rounded-full flex items-center space-x-1'>
                <span>{skill}</span>
                <button type='button' onClick={()=>handleRemoveSkill(skill)} className='text-secondary-foreground hover:text-red-500 focus:outline-none'><X size={14}/></button>
              </div>
            ))}
          </div>
        </div>
