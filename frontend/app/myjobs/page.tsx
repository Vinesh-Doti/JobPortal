    ...
    <div>
      ...
      <div className='mt-8 w-[90%] mx-auto flex flex-col'>
       ...
       <button className={`border border-gray-400 px-8 py-2 rounded-full font-medium
          ...
          `}onClick={()=>{setActiveTab("posts")}}>My Job Posts</button>
          <button className={`border border-gray-400 px-8 py-2 rounded-full font-medium
          ${
            activeTab==="likes"
            ? "border-transparent bg-[#7263f3] text-white"
          ...
          `} onClick={()=>setActiveTab("likes")}>Liked Jobs</button>
       </div>

      {activeTab ==="posts" && userJobs.length===0 && (<div>
        <p className='text-2xl font-bold'>No Jobs found.</p>
      </div>

      )}
      {activeTab==="likes" && likedJobs.length===0 && (<div>
