...
export const createJob = asyncHandler(async (req, res) => {
    ...
    if (!skills) return res.status(400).json({ message: "Skills is required" });
    if (!salaryType) return res.status(400).json({ message: "Salary type is required" });
    if (negotiable === undefined) return res.status(400).json({ message: "Negotiable is required" });
...
export const likeJob=asyncHandler(async(req,res)=>{
  try {
    ...
    console.log("Error in like Job: ",error);
    return res.status(500).json({
      message:"Server Error",
    })
  }
